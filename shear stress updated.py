...
for k in range(0, 11, 1):
    f  = k/10
    #print (f)
    #for c_front in range(1, 6, 1):
        #t_front = c_front/100
        #print (t_front)
        #for c_middle in range(1, 6, 1):
            #t_middle = c_middle/100
            #for c_rear in range(1, 6, 1):
                #t_rear = c_rear/100
                #for c_skin in range(1, 3, 1):
                    #t_skin = c_skin/100
    max0 = -1
    max1 = -1
    max2 = -1
    ...
    for j in range(3):
        ...
        if sw == 0:
            ...
            if abs(max([max(front_stress, key = abs), max(middle_stress, key = abs), max(rear_stress, key = abs), max(skin_stress_q1, key = abs), max(skin_stress_q2, key = abs)], key=abs)) > max0:
                max0 = abs(max([max(front_stress, key = abs), max(middle_stress, key = abs), max(rear_stress, key = abs), max(skin_stress_q1, key = abs), max(skin_stress_q2, key = abs)], key=abs))
            ...
            if abs(max([max(front_stress, key = abs), max(middle_stress, key = abs), max(rear_stress, key = abs), max(skin_stress_q1, key = abs), max(skin_stress_q2, key = abs)], key=abs)) > max1:
                max1 = abs(max([max(front_stress, key = abs), max(middle_stress, key = abs), max(rear_stress, key = abs), max(skin_stress_q1, key = abs), max(skin_stress_q2, key = abs)], key=abs))
            ...
            print("The maximum shear stress due to sw", j, "is" , max(front_stress, key = abs), max(middle_stress, key = abs), max(rear_stress, key = abs), max(skin_stress_q1, key = abs), max(skin_stress_q2, key = abs), "MPa")
            if abs(max([max(front_stress, key = abs), max(middle_stress, key = abs), max(rear_stress, key = abs), max(skin_stress_q1, key = abs), max(skin_stress_q2, key = abs)], key=abs)) > max2:
                max2 = abs(max([max(front_stress, key = abs), max(middle_stress, key = abs), max(rear_stress, key = abs), max(skin_stress_q1, key = abs), max(skin_stress_q2, key = abs)], key=abs))
        ...
        #print(max([max0, max1, max2]))
    if max([max0, max1, max2]) < max_stress:
        max_stress = max([max0, max1, max2])
        f_ideal = f
        t_front_ideal = t_front
        t_middle_ideal = t_middle
        t_rear_ideal = t_rear
        t_skin_ideal = t_skin

print (max_stress)
print ("The ideal positioning of the middle spar is", f_ideal)
